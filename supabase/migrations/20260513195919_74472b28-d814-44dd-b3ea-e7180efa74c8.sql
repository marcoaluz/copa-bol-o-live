...
BEGIN
  ...
  SELECT id INTO v_torneio_id FROM public.torneios WHERE slug='copa-2026' LIMIT 1;
  IF v_torneio_id IS NULL THEN
    SELECT id INTO v_torneio_id FROM public.torneios ORDER BY ordem LIMIT 1;
  END IF;
  IF v_torneio_id IS NULL THEN
    RAISE EXCEPTION 'Nenhum torneio disponível para self-test';
  END IF;

  -- Suprime triggers (incl. on_auth_user_created e set_torneio_id_from_partida)
  SET LOCAL session_replication_role = 'replica';

  -- Cleanup de execuções anteriores (ordem: filhos antes de pais)
  DELETE FROM public.apostas WHERE partida_id IN (SELECT id FROM public.partidas WHERE selecao_casa_id IN (SELECT id FROM public.selecoes WHERE codigo_iso ~ '^T[0-9][AB]$'));
  DELETE FROM public.apostas_placar WHERE partida_id IN (SELECT id FROM public.partidas WHERE selecao_casa_id IN (SELECT id FROM public.selecoes WHERE codigo_iso ~ '^T[0-9][AB]$'));
  ...
  BEGIN
    ...
    INSERT INTO partidas (fase, selecao_casa_id, selecao_visitante_id, data_hora, status, torneio_id, codigo_partida)
      VALUES ('grupos', v_sel_a, v_sel_b, now() + interval '10 days', 'agendada', v_torneio_id, 'TEST-'||substr(gen_random_uuid()::text,1,12)) RETURNING id INTO v_part_id;
    ...
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u1,v_part_id,'casa',1000,v_torneio_id);
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u2,v_part_id,'visitante',9000,v_torneio_id);
    ...
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u1,v_part_id,'casa',500,v_torneio_id);
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u2,v_part_id,'casa',10000,v_torneio_id);
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u3,v_part_id,'casa',2000,v_torneio_id);
    DECLARE up uuid;
    BEGIN
      FOR i IN 1..4 LOOP
        up := gen_random_uuid();
        ...
        INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES
          (up,v_part_id,'visitante', CASE i WHEN 1 THEN 50000 WHEN 2 THEN 50000 WHEN 3 THEN 50000 ELSE 47500 END, v_torneio_id);
      END LOOP;
    END;
    ...
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u1,v_part_id,'casa',1500,v_torneio_id);
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (u2,v_part_id,'visitante',2500,v_torneio_id);
    ...
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id,created_at) VALUES (u1,v_part_id,'casa',1000,v_torneio_id, now() - interval '3 minutes');
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id,created_at) VALUES (u2,v_part_id,'casa',1000,v_torneio_id, now() - interval '2 minutes');
    INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id,created_at) VALUES (u3,v_part_id,'casa',1000,v_torneio_id, now() - interval '1 minute');
    DECLARE up uuid;
    BEGIN
      up := gen_random_uuid();
      INSERT INTO auth.users (id) VALUES (up);
      INSERT INTO profiles (id, apelido, saldo_centavos) VALUES (up, 'tst4p_' || substr(up::text,1,8), 1000000);
      INSERT INTO apostas (usuario_id,partida_id,palpite,valor_centavos,torneio_id) VALUES (up,v_part_id,'visitante',7000,v_torneio_id);
    END;
