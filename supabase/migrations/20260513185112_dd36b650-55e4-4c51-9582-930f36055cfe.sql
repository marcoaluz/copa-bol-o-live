...
BEGIN
  ...
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst1u1', 100000 FROM au RETURNING id INTO u1;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst1u2', 100000 FROM au RETURNING id INTO u2;
  ...
  INSERT INTO partidas (fase, selecao_casa_id, selecao_visitante_id, data_hora, status, torneio_id)
    VALUES ('grupos', v_sel_a, v_sel_b, now() + interval '10 days', 'agendada', v_torneio_id) RETURNING id INTO v_part_id;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst2u1', 100000 FROM au RETURNING id INTO u1;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst2u2', 100000 FROM au RETURNING id INTO u2;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst2u3', 100000 FROM au RETURNING id INTO u3;
  ...
  BEGIN
    FOR i IN 1..4 LOOP
      WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst2p'||i, 1000000 FROM au RETURNING id INTO up;
  ...
  INSERT INTO partidas (fase, selecao_casa_id, selecao_visitante_id, data_hora, status, torneio_id)
    VALUES ('grupos', v_sel_a, v_sel_b, now() + interval '10 days', 'agendada', v_torneio_id) RETURNING id INTO v_part_id;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst3u1', 0 FROM au RETURNING id INTO u1;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst3u2', 0 FROM au RETURNING id INTO u2;
  ...
  INSERT INTO partidas (fase, selecao_casa_id, selecao_visitante_id, data_hora, status, torneio_id)
    VALUES ('grupos', v_sel_a, v_sel_b, now() + interval '10 days', 'agendada', v_torneio_id) RETURNING id INTO v_part_id;
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst4u1', 0 FROM au RETURNING id INTO u1;
  PERFORM pg_sleep(0.01);
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst4u2', 0 FROM au RETURNING id INTO u2;
  PERFORM pg_sleep(0.01);
  WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst4u3', 0 FROM au RETURNING id INTO u3;
  ...
  DECLARE up uuid;
  BEGIN
    WITH au AS (INSERT INTO auth.users (id) VALUES (gen_random_uuid()) RETURNING id) INSERT INTO profiles (id, apelido, saldo_centavos) SELECT id, 'tst4p', 1000000 FROM au RETURNING id INTO up;
